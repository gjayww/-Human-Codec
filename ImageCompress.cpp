...
int compress(const string& fileName){
    ...
    unsigned int totalPixels = wid * hei;
    double bitrate = static_cast<double>(hufSize) / totalPixels;
    cout << "編碼位元率 (bits per pixel): " << bitrate << endl;
    releaseData();
}


int main(){
    readbmpfile("lenna.bmp");
    compress("lenna.bmp.hfm");
    return 0;
}
